...
import {
  assignmentEmployeesSelect,
  generalScheduleSelect,
  requiredShiftConfigsSelect,
  scheduleRoomsSelect,
  scheduleShiftsSelect,
} from "../schedule.queries.js";
import { addDaysIso } from "../common/schedule-date.js";
import {
  ...
  mapScheduleRoom,
  mapShift,
} from "../common/schedule.mappers.js";
...
export async function getAssignmentPayload(weekStart) {
  const [roomsResult, shiftsResult, schedulesResult, employeesResult, requiredShiftsResult] =
    await Promise.all([
      pool.query(scheduleRoomsSelect),
      pool.query(scheduleShiftsSelect),
      pool.query(generalScheduleSelect, [weekStart]),
      pool.query(assignmentEmployeesSelect),
      pool.query(requiredShiftConfigsSelect),
    ]);
  ...
  return {
    ...
    employees: employeesResult.rows.map(mapAssignmentEmployee),
    required_shifts: requiredShiftsResult.rows.map(mapRequiredShiftConfig),
