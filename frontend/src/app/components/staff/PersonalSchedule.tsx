...
function getShiftClass(shift: ShiftColumn) {
  ...
  if (key.includes("TOI") || key.includes("TỐI")) return "text-blue-600";
...
export function PersonalSchedule({ user }: PersonalScheduleProps) {
  ...
  function handleCellClick(date: string, shiftCode: string) {
          ...
          <table className="w-full text-sm border-collapse min-w-[760px]">
            <thead>
              <tr className="bg-gray-50">
                <th className="border border-gray-200 px-4 py-2.5 text-left font-semibold text-gray-700 w-28">
                  Ngày
                </th>
                {shiftColumns.map((shift) => (
                  <th
                    key={shift.code}
                    className={`border border-gray-200 px-3 py-2.5 text-center font-semibold ${getShiftClass(shift)}`}
                  >
                    <div>{shift.name}</div>
            ...
            <tbody>
              {days.map((day) => (
                <tr key={day.isoDate} className="hover:bg-gray-50">
      ...
      {selectedData && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-800">Chi tiết ca trực</h3>
            <button type="button" onClick={() => setSelectedCell(null)}>
              <X size={16} className="text-gray-400" />
            </button>
          </div>
          <div className="grid grid-cols-3 gap-4 text-sm">
            <InfoRow label="Ngày trực" value={formatLongDate(selectedData.dutyDate)} />
            <InfoRow label="Ca trực" value={selectedData.shiftName} />
            <InfoRow label="Giờ trực" value={formatTimeRange({ startTime: selectedData.startTime, endTime: selectedData.endTime })} />
            <InfoRow label="Khoa" value={selectedData.departmentName} />
            <InfoRow label="Phòng" value={formatRoom(selectedData.roomCode, selectedData.roomName)} />
            <InfoRow label="Người phân công" value={selectedData.assignedByUsername || "-"} />
            <div className="flex items-start gap-2">
              <span className="text-sm text-gray-500 w-28 flex-shrink-0">Trạng thái:</span>
              <StatusBadge status={getScheduleBadge(selectedData.status)} />
            </div>
            <InfoRow label="Ghi chú" value={selectedData.note || "-"} />
          </div>
        </div>
      )}
    </div>
